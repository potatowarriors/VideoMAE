...
def load_origvit_bidir_weights(model, vit_path ,args):
    if args.vmae_finetune.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(
            args.vmae_finetune, map_location='cpu', check_hash=True)
    else:
    ...
    print("Load VideoMAE ckpt from %s" % args.vmae_finetune)
    checkpoint_model = None
    checkpoint_vit = torch.load(vit_path, map_location='cpu')
    print("Load ViT ckpt from %s" % args.vit_finetune)
    for model_key in args.model_key.split('|'):
        if model_key in checkpoint:
            checkpoint_model = checkpoint[model_key]
            print("Load state_dict by model_key = %s" % model_key)
    ...
    all_keys = list(checkpoint_model.keys())
    vit_all_keys = list(checkpoint_vit.keys())
    new_dict = OrderedDict()
    for key in all_keys:
        if key.startswith('backbone.'):
            new_dict[key[9:]] = checkpoint_model[key]
    ...
    # add new code for load clip weight
    for key in vit_all_keys:
        if key.startswith('blocks.'):
            if key[8] == '.':
                new_dict['blocks.'+ key[7] + '.vit_' + key[9:]] = checkpoint_vit[key]
            else : # layer10 ~ 11 process
                new_dict['blocks.'+ key[7:9] + '.vit_' + key[10:]] = checkpoint_vit[key]
        else:
            new_dict['vit_' + key] = checkpoint_vit[key]
...
def load_vmaex2_bidir_weights(model ,args):
    if args.vmae_finetune.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(
            args.vmae_finetune, map_location='cpu', check_hash=True)
    else:
    ...
    for key in all_keys:
        if key.startswith('backbone.'):
            ...
            if key[16] == '.':
                for i in range(1,3):
                    new_dict['blocks.'+ key[15] + '.vmae' + str(i) + '_' + key[17:]] = checkpoint_model[key]
            elif len(key) > 17:
                if key[17] == '.' : # layer10 ~ 11 process
                    for i in range(1, 3):
                        new_dict['blocks.'+ key[15:17] + '.vmae' + str(i) + '_' + key[18:]] = checkpoint_model[key]
                if len(key) > 19:
                    if key[19] == '.':
                        for i in range(1,3):
                            new_dict['vmae' + str(i) + '_' + key[8:]] = checkpoint_model[key]
            else:
                pass
        else:
            new_dict[key] = checkpoint_model[key]
