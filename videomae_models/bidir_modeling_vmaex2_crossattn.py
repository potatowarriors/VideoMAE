...
class CrossAttentionS2T(nn.Module):
    def __init__(self, dim: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.vmae_space_pos1 = nn.Parameter(self.scale * torch.randn((196, dim)))
        self.vmae_space_pos2 = nn.Parameter(self.scale * torch.randn((196, dim)))
    ...
    def s2t_cross_attn(self, s_x, t_x): # s_x=[n (b t) d], t_x=[b n d]
        ...
        s_x = s_x + self.vmae_space_pos1
        t_x = rearrange(t_x, 'b (t n) d -> (b t) n d', t=8)
        t_x = t_x + self.vmae_space_pos2
...
class CrossAttentionT2S(nn.Module):
    def __init__(self, dim: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.scale = head_dim ** -0.5
        all_head_dim = head_dim * self.num_head
        self.vmae1_time_pos = nn.Parameter(self.scale * torch.randn((8, dim)))
        self.vmae2_time_pos = nn.Parameter(self.scale * torch.randn((8, dim))) #왠지 original vmae에 nosie가 되는거같다 끄고 해보자.
    ...
    def t2s_cross_attn(self, s_x, t_x): # s_x=[n (b t) d], t_x=[b n d]
        ...
        s_x = s_x + self.vmae1_time_pos
        t_x = rearrange(t_x, 'b (t n) d -> (b n) t d', t=8)
        t_x = t_x + self.vmae2_time_pos
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        ############################ VMAE1 MHSA ###########################
        self.vmae1_norm1 = norm_layer(dim)
        self.vmae1_attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, attn_head_dim=attn_head_dim)
        self.vmae1_T_Adapter = Adapter(dim)
        ##################################################################
        
        ############################ VMAE2 MHSA ###########################
        self.vmae2_norm1 = norm_layer(dim)
        self.vmae2_attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, attn_head_dim=attn_head_dim)
        self.vmae2_T_Adapter = Adapter(dim)
        ##################################################################
        #########################################################################################
        ...
        ############################ VMAE1 FFN ###############################
        self.vmae1_norm2 = norm_layer(dim)
        self.vmae1_mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.vmae1_T_MLP_Adapter = Adapter(dim, skip_connect=False)
        #######################################################################
        
        ############################ VMAE2 FFN ###############################
        self.vmae2_norm2 = norm_layer(dim)
        self.vmae2_mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.vmae2_T_MLP_Adapter = Adapter(dim, skip_connect=False)
        #######################################################################
        #########################################################################################
    ...
    def forward(self,s_x, t_x):
        ...
        # VMAE1 Time MHSA
        s_x = s_x + self.vmae1_T_Adapter(self.vmae1_attn(self.vmae1_norm1(s_x)))
        
        # VMAE2 Time MHSA
        t_x = t_x + self.vmae2_T_Adapter(self.vmae2_attn(self.vmae2_norm1(t_x)))
        ########################################################################
        ...
        # VMAE1
        s_xn = self.vmae1_norm2(s_x)
        s_x = s_x + self.vmae1_mlp(s_xn) + self.drop_path(self.scale * self.vmae1_T_MLP_Adapter(s_xn))
        # VMAE2
        t_xn = self.vmae2_norm2(t_x)
        t_x = t_x + self.vmae2_mlp(t_xn) + self.drop_path(self.scale * self.vmae2_T_MLP_Adapter(t_xn))
        ############################################################################
...
class STCrossTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.vmae1_patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, num_frames=all_frames, tubelet_size=self.tubelet_size)
        self.vmae2_patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, num_frames=all_frames, tubelet_size=self.tubelet_size)
        num_patches = self.vmae1_patch_embed.num_patches
        ...
        if use_learnable_pos_emb:
            ...
            self.pos_embed1 = get_sinusoid_encoding_table(num_patches, embed_dim)
        ...
        self.vmae_fc_norm1 = norm_layer(embed_dim)
        self.vmae_fc_norm2 = norm_layer(embed_dim)
    ...
    def forward_features(self, x):
        ...
        ######################## VMAE1 spatial path ########################
        s_x = self.vmae1_patch_embed(x)

        if self.pos_embed1 is not None:
            s_x = s_x + self.pos_embed1.expand(B, -1, -1).type_as(s_x).to(s_x.device).clone().detach()
        s_x = self.pos_drop(s_x)
        
        ######################## VMAE spatial path #########################
        t_x = self.vmae2_patch_embed(x)

        if self.pos_embed2 is not None:
            t_x = t_x + self.pos_embed2.expand(B, -1, -1).type_as(t_x).to(t_x.device).clone().detach()
        ...
        s_x = self.vmae_fc_norm1(s_x.mean(1))
        t_x = self.vmae_fc_norm2(t_x.mean(1)) # all patch avg pooling
...
@register_model
def bidir_vmaex2_vit_base_patch16_224(pretrained=False, **kwargs):
...
@register_model
def compo_bidir_vmaex2_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
