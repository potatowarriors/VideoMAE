...
class CrossAttentionT2S(nn.Module): # 이게 VMAE로 치면 blocks class다. 여기에 cross s2t_attn layer가 추가되어야 한다.
    ...
    def t2s_cross_attn(self, s_x, t_x):
        B, _, _ = t_x.shape
        s_x, s_x_cls = s_x[1:,:,:], s_x[0,:,:]
        s_x = rearrange(s_x, 'n (b t) d -> (b n) t d', b=B) # batch -> token
        ...
        s_x = (t2s_attn @ t2s_v)
        s_x = rearrange(s_x, 'b h t d -> b t (h d)')
        s_x = self.t2s_proj(s_x)
        s_x = rearrange(s_x,'(b n) t d -> n (b t) d', b=B, t=8)
        s_x = torch.cat((s_x_cls.unsqueeze(0), s_x), 0)
        return s_x
...
class Block(nn.Module):
    ...
    def forward(self, s_x):
        ...
        ## FFN
        xn = self.clip_ln_2(s_x)
        s_x = s_x + self.clip_mlp(xn) + self.drop_path(self.scale * self.MLP_Adapter(xn))
...
class STCrossTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        #     self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))1212 
        # else:
