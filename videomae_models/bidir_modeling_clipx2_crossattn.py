...
class CrossAttentionS2T(nn.Module):
    def __init__(self, dim: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.clip1_space_pos = nn.Parameter(self.scale * torch.randn((197, dim)))
        self.clip2_space_pos = nn.Parameter(self.scale * torch.randn((197, dim)))
    ...
    def s2t_cross_attn(self, s_x, t_x): # s_x=[n (b t) d], t_x=[b n d]
        B, _, _ = t_x.shape
        s_x = rearrange(s_x, 'n b d -> b n d') # batch -> token
        s_x = s_x + self.clip1_space_pos
        t_x = rearrange(t_x, 'n b d -> b n d')
        t_x = t_x + self.clip2_space_pos
...
class CrossAttentionT2S(nn.Module):
    def __init__(self, dim: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.scale = head_dim ** -0.5
        all_head_dim = head_dim * self.num_head
        self.clip1_time_pos = nn.Parameter(self.scale * torch.randn((16, dim)))
        self.clip2_time_pos = nn.Parameter(self.scale * torch.randn((16, dim))) #왠지 original vmae에 nosie가 되는거같다 끄고 해보자.
    ...
    def t2s_cross_attn(self, s_x, t_x): # s_x=[n (b t) d], t_x=[b n d]
        B, _, _ = t_x.shape
        s_x = rearrange(s_x, 'n (b t) d -> (b n) t d', t=16) # batch -> token
        s_x = s_x + self.clip1_time_pos
        t_x = rearrange(t_x, 'n (b t) d -> (b n) t d', t=16)
        t_x = t_x + self.clip2_time_pos
        ...
        s_x = rearrange(s_x,'(b n) t d -> n (b t) d', n=197)
        return s_x
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        ###################################### MHSA code #####################################
        ############################ CLIP1 MHSA ###########################
        self.clip1_ln_1 = LayerNorm(dim)
        self.clip1_attn = nn.MultiheadAttention(dim, num_heads)
        self.S_Adapter = Adapter(dim)
        ##################################################################
        
        ############################ CLIP2 MHSA ###########################
        self.clip2_ln_1 = LayerNorm(dim)
        self.clip2_attn = nn.MultiheadAttention(dim, num_heads)
        ...
        ###################################### FFN code #########################################
        ############################ CLIP 1 FFN ###############################
        self.clip1_ln_2 = LayerNorm(dim)
        self.clip1_mlp = nn.Sequential(OrderedDict([
        ...
        ############################ CLIP 2 FFN ###############################
        self.clip2_ln_2 = LayerNorm(dim)
        self.clip2_mlp = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(dim, dim * 4)),
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(dim * 4, dim))
        ]))
    ...
    def attention1(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.clip1_attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
    def attention2(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.clip2_attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
    ...
    def forward(self,s_x, t_x):
        ...
        ############################ MHSA Forward #############################
        # CLIP 1 Space MHSA
        s_x = s_x + self.S_Adapter(self.attention1(self.clip1_ln_1(s_x))) # original space multi head self attention
        # CLIP 2 Space MHSA
        t_x = t_x + self.T_Adapter(self.attention2(self.clip2_ln_1(t_x))) # original space multi head self attention
        ########################################################################
        ...
        ############################ FFN Forward ##################################
        # CLIP 1
        s_xn = self.clip1_ln_2(s_x)
        s_x = s_x + self.clip1_mlp(s_xn) + self.drop_path(self.scale * self.S_MLP_Adapter(s_xn))
        # CLIP 2
        t_xn = self.clip2_ln_2(t_x)
        t_x = t_x + self.clip2_mlp(t_xn) + self.drop_path(self.scale * self.S_MLP_Adapter(t_xn))
        ############################################################################
...
class STCrossTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        scale = embed_dim ** -0.5
        ###################################### CLIP 1 #####################################
        self.clip1_conv1 = nn.Conv2d(in_channels=3, out_channels=embed_dim, kernel_size=patch_size, stride=patch_size, bias=False)
        self.clip1_class_embedding = nn.Parameter(scale * torch.randn(embed_dim))
        self.clip1_positional_embedding = nn.Parameter(scale * torch.randn((img_size // patch_size) ** 2 + 1, embed_dim))
        self.clip1_ln_pre = LayerNorm(embed_dim)
        ######################################################################################
        
        ###################################### CLIP 2 #####################################
        self.clip2_conv1 = nn.Conv2d(in_channels=3, out_channels=embed_dim, kernel_size=patch_size, stride=patch_size, bias=False)
        self.clip2_class_embedding = nn.Parameter(scale * torch.randn(embed_dim))
        self.clip2_positional_embedding = nn.Parameter(scale * torch.randn((img_size // patch_size) ** 2 + 1, embed_dim))
        self.clip2_ln_pre = LayerNorm(embed_dim)
        ######################################################################################
...
#             self.pos_embed = get_sinusoid_encoding_table(num_patches, embed_dim)
        ...
        self.clip1_ln_post = LayerNorm(embed_dim)
        self.clip2_ln_post = LayerNorm(embed_dim)
    ...
    def forward_features(self, x):
        ...
        s_x = self.clip1_conv1(s_x) # shape = [*, embeddim, grid, grid]
        s_x = s_x.reshape(s_x.shape[0], s_x.shape[1], -1) # [*, embeddim, grid**2]
        s_x = s_x.permute(0, 2, 1) # shape[batch, patchnum, embeddim]
        s_x = torch.cat([self.clip1_class_embedding.to(s_x.dtype) + torch.zeros(s_x.shape[0], 1, s_x.shape[-1], dtype=s_x.dtype, device=s_x.device), s_x], dim=1)
        s_x = s_x + self.clip1_positional_embedding.to(s_x.dtype)
        s_x = self.clip1_ln_pre(s_x)
        #####################################################################
        
        ######################## VMAE spatial path #########################
        t_x = x
        t_x = rearrange(t_x, 'b c t h w -> (b t) c h w')
        t_x = self.clip2_conv1(t_x) # shape = [*, embeddim, grid, grid]
        t_x = t_x.reshape(t_x.shape[0], t_x.shape[1], -1) # [*, embeddim, grid**2]
        t_x = t_x.permute(0, 2, 1) # shape[batch, patchnum, embeddim]
        t_x = torch.cat([self.clip2_class_embedding.to(t_x.dtype) + torch.zeros(t_x.shape[0], 1, t_x.shape[-1], dtype=t_x.dtype, device=t_x.device), t_x], dim=1)
        t_x = t_x + self.clip2_positional_embedding.to(t_x.dtype)
        t_x = self.clip2_ln_pre(t_x)
        #####################################################################
        ...
        s_x = self.clip1_ln_post(s_x[:,:,0,:].mean(1)) # all cls tokens avg pooling
        t_x = rearrange(t_x, '(b t) n d -> b t n d', b=B)
        t_x = self.clip2_ln_post(t_x[:,:,0,:].mean(1)) # all cls tokens avg pooling
...
@register_model
def bidir_clipx2_vit_base_patch16_224(pretrained=False, **kwargs):
...
@register_model
def compo_bidir_clipx2_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
