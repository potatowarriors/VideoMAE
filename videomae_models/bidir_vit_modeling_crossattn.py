...
class Block(nn.Module):
    ...
    def forward(self,s_x, t_x):
        ...
        ############################ MHSA Forward #############################
        # AIM Space MHSA
        s_x = s_x + self.S_Adapter(self.vit_attn(self.vit_norm1(s_x))) # original space multi head self attention
        # VMAE Time MHSA
        ...
        ############################ FFN Forward ##################################
        s_xn = self.vit_norm2(s_x)
        s_x = s_x + self.vit_mlp(s_xn) + self.drop_path(self.scale * self.S_MLP_Adapter(s_xn))
...
@register_model
def bidir_orig_vit_base_patch16_224(pretrained=False, **kwargs):
...
@register_model
def compo_bidir_orig_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
...
@register_model
def compo_bidir_mae_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, **kwargs)
    #model.default_cfg = _cfg()
